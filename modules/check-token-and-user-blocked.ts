...
export default async function FirebaseAuthSimplePolicy(
    ...
    try {
        ...
        try {
            ...
            return new Response(
                JSON.stringify({ error: "Invalid token juji" }),
                { status: 401, headers: { "Content-Type": "application/json" } }
            );
        }
        ...
        const userServiceUrl = `https://users-service-production-968d.up.railway.app/users/${userId}`;
